...
The Go client is a **Dynamic Cobra Proxy**. It Fetch the command schema from the daemon at runtime (or from cache) to build its CLI tree, while remaining a thin tunnel for command execution.
